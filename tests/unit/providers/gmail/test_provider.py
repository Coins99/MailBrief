...
async def test_resumes_from_continuation_token(respx_mock: respx.MockRouter) -> None:
    window = local_day_window(NOW, ZoneInfo("America/Toronto"))
    listing = respx_mock.get(MESSAGES_URL).respond(json={"messages": [{"id": "r1"}]})
    respx_mock.get(MESSAGES_URL + "/r1").respond(json=metadata("r1", received=window.start_utc))
    async with httpx.AsyncClient() as http:
        provider = GmailProvider(FakeSession(), GmailClient(http, FakeSession(), sleep=no_sleep))
        await provider.connect()
        pages = [
    ...
    assert [[m.provider_message_id for m in p.messages] for p in pages] == [["r1"]]
    assert listing.calls[0].request.url.params["pageToken"] == "resume-token"
